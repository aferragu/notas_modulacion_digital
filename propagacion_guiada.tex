...
\begin{itemize}
    \item $R(f)$ [$\Omega/\unit{\meter}$]: resistencia serie de los conductores, debida a su conductividad finita.
    ...
    \item $G(f)$ [$\unit{\siemens}/\unit{\meter}$]: conductancia paralelo, debida a las pérdidas (corrientes de fuga) en el dieléctrico que separa los conductores.
...
\begin{figure}
    \begin{center}
        \begin{circuitikz}[scale=1.15, european]
            \draw (0,0) to[R=$R\Delta z$] (1.9,0) to[L=$L\Delta z$] (3.6,0);
            \draw (3.6,0) -- (4.4,0);
            \draw (3.6,0) to[R=$1/G\Delta z$] (3.6,-1.1) to[C=$C\Delta z$] (3.6,-2.2);
            \draw (0,-2.2) -- (3.6,-2.2);
            \draw (3.6,-2.2) -- (4.4,-2.2);
            \draw (0,-2.2) -- (0,0);
            \draw[-latex, thick] (-0.6,-1.85) -- (-0.6,-0.35);
            \node[left] at (-0.6,-1.1) {$v(z,t)$};
            \draw[-latex, thick] (0.15,0.75) -- (1.15,0.75);
            \node[above] at (0.65,0.75) {\footnotesize $i(z,t)$};
            \node[below left] at (0,-2.2) {\footnotesize $z$};
            \node[below right] at (4.4,-2.2) {\footnotesize $z+\Delta z$};
        \end{circuitikz}
    \end{center}
...
\begin{equation}\label{eq:telegrafista}
    \begin{aligned}
        \frac{\partial v(z,t)}{\partial z} &= -R\, i(z,t) - L\, \frac{\partial i(z,t)}{\partial t}, \\
        \frac{\partial i(z,t)}{\partial z} &= -G\, v(z,t) - C\, \frac{\partial v(z,t)}{\partial t}.
    \end{aligned}
\end{equation}
Estas ecuaciones, planteadas por Kelvin y luego completadas por Heaviside en el contexto del cable telegráfico transatlántico, son el punto de partida de todo el análisis de líneas de transmisión: relacionan cómo cambia la tensión (corriente) en el espacio con cómo cambia la corriente (tensión) en el tiempo, acoplando así la dimensión espacial que el modelo de parámetros concentrados ignoraba.

\section{Ecuación de onda y propagación en la línea}\label{sec:ecuacion de onda}

Combinando las dos ecuaciones de \eqref{eq:telegrafista} -- derivando la primera respecto de $z$ y sustituyendo la segunda -- se obtiene una única ecuación de segundo orden para $v(z,t)$ (y, de forma simétrica, otra para $i(z,t)$):
\begin{equation*}
    \frac{\partial^2 v(z,t)}{\partial z^2} = LC\, \frac{\partial^2 v(z,t)}{\partial t^2} + (RC+LG)\, \frac{\partial v(z,t)}{\partial t} + RG\, v(z,t),
\end{equation*}
la \emph{ecuación de onda} de la línea con pérdidas. En el caso ideal sin pérdidas ($R=G=0$) se reduce a la ecuación de onda clásica $\partial^2 v/\partial z^2 = LC\, \partial^2 v/\partial t^2$, cuya solución general es $v(z,t) = f(t-z/v_0) + g(t+z/v_0)$ con $v_0=1/\sqrt{LC}$: dos ondas viajando sin distorsión, una en cada sentido. Con pérdidas, sin embargo, esta ecuación no tiene en general una solución tan simple en el dominio temporal.

Como el sistema \eqref{eq:telegrafista} es lineal e invariante en el tiempo (para $z$ fijo, $R,L,G,C$ no dependen de $t$), conviene resolverlo trabajando frecuencia a frecuencia, tal como hicimos para los sistemas LTI en capítulos anteriores. Tomando transformada de Fourier en $t$ a ambos miembros de \eqref{eq:telegrafista}, con $v(z,t) \fourier \hat{V}(z,f)$ e $i(z,t) \fourier \hat{I}(z,f)$, y usando que derivar en $t$ equivale a multiplicar por $j2\pi f$, se obtiene, para cada frecuencia $f$, un sistema de ecuaciones diferenciales \emph{ordinarias} en $z$:
...
\end{equation}
tomando la raíz cuadrada con la determinación que hace $\alpha(f)\geqslant 0$ (una elección física: las ondas se atenúan al propagarse, no crecen). Llamamos a $\alpha(f)$ la \emph{constante de atenuación} (en $\unit{\neper\per\meter}$, o convertida a $\unit{\deci\bel\per\meter}$ multiplicando por $20\log_{10}\e \approx 8.686$) y a $\beta(f)$ la \emph{constante de fase} (en $\unit{\radian\per\meter}$).

La ecuación diferencial resultante, $d^2\hat V/dz^2 = \gamma(f)^2 \hat V(z,f)$, tiene solución general:
\begin{equation}\label{eq:solucion V}
    \hat{V}(z,f) = \hat{V}^+(f)\, \e^{-\gamma(f) z} + \hat{V}^-(f)\, \e^{\gamma(f) z}.
\end{equation}
Interpretemos cada término recordando que $\e^{j2\pi f t}$ es nuestra convención para una sinusoide (Sección \ref{sec:parametros distribuidos} en adelante venimos usando $\fourier$ con esta convención). El primer término, $\hat{V}^+(f)\e^{-\alpha(f) z}\e^{-j\beta(f) z}$, se atenúa a medida que $z$ crece y acumula un atraso de fase creciente con $z$: corresponde, al antitransformar, a una onda que se propaga en el sentido de $z$ creciente y llega retardada y atenuada -- la llamamos \emph{onda incidente}. El segundo término, con $\e^{+\gamma(f) z}$, crece con $z$: no es físico para una línea semi-infinita, pero si la línea tiene longitud finita y termina en una carga que refleja parte de la energía (Sección \ref{sec:linea cargada}), representa la \emph{onda reflejada}, que se propaga en el sentido de $z$ decreciente y por lo tanto se atenúa a medida que $z$ decrece.
...
Con esto cerramos el círculo que abrió este capítulo: partiendo de los parámetros $R,L,G,C$ de la línea (Sección \ref{sec:parametros distribuidos}), llegamos a la constante de propagación y a $\hat H(f)$ (Secciones \ref{sec:ecuacion de onda} y \ref{sec:dispersion pulsos}), entendimos el rol de la carga y la reflexión (Sección \ref{sec:linea cargada}), y finalmente le dimos contenido físico tanto a la distorsión que produce ISI (Capítulo \ref{chapter:modulacion banda base}) como a la atenuación que limita el alcance de un enlace (Capítulo \ref{chapter:deteccion ruido}). En el próximo capítulo retomamos el filtro ecualizador que \S3.6 dejó apenas esbozado, ahora motivados por un $\hat H(f)$ con origen físico concreto, y avanzamos hacia el problema, igualmente pendiente, del sincronismo.
