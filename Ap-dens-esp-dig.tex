...
\section{Densidad espectral de potencia de la señal PAM}\label{appendix:pam}
...
Para estudiar a la se\~nal $x(t)$ mediante herramientas de procesos estacionarios (de tiempo continuo), será además
necesario ``des-sincronizar'' el análisis introduciendo un retardo aleatorio, al igual que se hizo en la Sección \ref{sec.estoc}.
...
\end{itemize}
La ergodicidad implica que la autocorrelación estocástica $R_a(l)$  coincide con probabilidad 1
...
donde también se introdujo su Transformada de Fourier de Tiempo
...
\end{align}
Esta función, periódica de período 1, depende de la variable de frecuencia normalizada $\varphi$.


\subsection*{Teorema}
Sea $\{a_k\}$ un proceso estacionario de símbolos, con densidad espectral de potencia discreta $
...
Sustiyendo en (\ref{eq.dig}) llegamos a la fórmula utilizada en la Sección \ref{sec.densdig}.
\begin{equation}
    \label{eq:dig3}
