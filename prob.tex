...
\begin{minipage}{0.3\textwidth}La función de distribución de una V.A. se define como $F_X(x)=P\left(\{X\leq x\}\right)$\end{minipage}
\begin{minipage}{0.65\textwidth}
    \resizebox{\textwidth}{!}{
    \bpgf{Imagenes/prob/prob-1}
    \begin{tikzpicture}
        \draw [->] (-5,0) -- (5,0) node [anchor=north] {$x$};
        \draw [->] (0,-0.5) -- (0,1.5) node [anchor=west] {$F_X(x)$};
