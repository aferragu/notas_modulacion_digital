\section{Modulaci\'on pasabanda $M$-aria y su espectro de potencia}
...
para la modulaci\'on pasabanda anal\'ogica. Recordamos que se plantea
la relaci\'on
...
En el caso de la modulaci\'on digital, se considera una envolvente compleja
\[
...
multiplicados por s\'imbolos complejos $c_k=a_k + j b_k$, que
var\'ian en un conjunto discreto llamado
 {\em constelación de s\'imbolos }.
%
...
En el caso general $M$-ario, se puede obtener tambi\'en la densidad espectral de potencia  a partir de
la de su envolvente compleja, generalizando la f\'ormula (\ref{eq:dep}).
\[
...
donde suponemos que los s\'imbolos $\{c_k\}$ son variables aleatorias complejas, independientes e
identicamente distribuidas. Suponemos, sin perder generalidad, que $E[c_k]=0$, es decir que la
costelaci\'on es balanceada alrededor del cero, con puntos equiprobables.

\newpage

La respuesta es que se puede utilizar la misma f\'ormula de antes (con $\mu=0$):
\[
    S_{x_{ce}} = \sigma_c^2 f_s |\hat{P}(f)|^2,
        \]
donde ahora $\sigma_c^2 = E[|c_k|^2] = E[a_k^2 + b_k^2]$, varianza del m\'odulo de los s\'imbolos.
Con este m\'etodo consideramos los dos casos principales.
...
que es el mismo resultado que en BPSK, independiente del n\'umero de puntos $M$ de la constelaci\'on.

\newpage

\subsubsection*{Modulación QAM}

Aqu\'i la constelaci\'on se describe en coordenadas cartesianas, $c_k = a_k+j b_k$, donde
...
Tenemos entonces que $\sigma_c^2 = \sigma_a^2+\sigma_b^2$, y cada uno de estos t\'erminos se calcula como
\[
