...
\begin{equation*}
    R_x(\tau) = \frac{1}{T_s} p_0(t/T_s) * p_0(t/T_s),
\end{equation*}
siendo $p_0(t)$ el pulso de altura $1$ y ancho $1$ centrado en $0$ (por lo tanto, $p_0(t/T_s)$ es un pulso de ancho $T_s$). Usando propiedades de la transformada de Fourier obtenemos:
\begin{equation*}
    p_0(t/T_s) \fourier T_s \sinc(\pi T_s f) \quad \Longrightarrow \quad R_x(\tau) \fourier T_s \sinc^2(\pi T_s f)
\end{equation*}
Las mismas se grafican en la Figura \ref{fig:ejemplo dep continua}. Notar que en el segundo caso, el $90\%$ de la energía está contenida en el primer lóbulo, es decir en el intervalo $[-f_s,f_s]$, siendo $f_s=1/T_s$ la frecuencia de símbolo.
...
\begin{equation}\label{eq:media del filtrado}
    \E{y(t)} = \E{\int_{-\infty}^\infty h(s)x(t-s)ds} = \int_{-\infty}^\infty h(s)\underbrace{\E{x(t-s)}}_{\mu_x}ds = \mu_x  \int_{-\infty}^\infty ds.
\end{equation}
...
\begin{align*}
    ...
    &=\E{\int_{-\infty}^\infty\int_{-\infty}^\infty h(u)h(t-\tau-v)x(t-u)x(v)dvdu} \\
    &=\int_{-\infty}^\infty\int_{-\infty}^\infty h(u)h(t-\tau-v)\underbrace{\E{x(t-u)x(v)}}_{R_x(t-u-v)} dv \\
...
\begin{ejemplo}
    Supongamos que la onda binaria aleatoria del Ejemplo \ref{ej:onda binaria aleatoria} se pasa por un filtro pasabajos ideal, de ganancia $1$ y frecuencia de corte $f_s$. Utilizando la ecuación \eqref{eq:espectro filtrado} podemos calcular el espectro a la salida de la siguiente forma:
    \begin{equation*}
        S_y(f) = |\hat{H}(f)|^2 S_x(f) = T_s \sinc(\pi T_s f) \mathbf{1}_{[-f_s,f_s]}(f).
    \end{equation*}
