...
\section{Procesos estocásticos de tiempo discreto}
...
\section{Procesos estocásticos de tiempo continuo}
...
\section{Filtrado de procesos estacionarios}
...
\section{Modulación de procesos estacionarios}
...
\begin{equation*}
    S_{x_c}(f) = S_x(f) *\left[\frac{\delta(f-f_c) + \delta(f+f_c)}{2}\right] = \frac{1}{2}\left[S_x(f-f_c) + S_x(f+f_c)\right].
\end{equation*}
Es decir, la densidad se traslada a las frecuencias de modulación.

\section{Procesos gaussianos y Ruido blanco gaussiano}
...
\begin{ejemplo}[Ruido blanco filtrado]
    ...
    \begin{equation}\label{eq:cola ruido pasabajos}
        \p{n(t)>\gamma} = \p{\frac{n(t)}{\sqrt{\eta_0 B}} > \frac{\gamma}{\sqrt{\eta_0 B}}} = 1-Q\left(\frac{\gamma}{\sqrt{\eta_0 B}}\right),
    \end{equation}
