...
R_y(\tau)&=\ R_x(\tau);\\
                ...
                \tag{\#}
\end{equation*}
La primera condición, ya impuesta en el Capítulo \ref{chapter:procesos estocasticos},  implica que $x(t)$, $y(t)$ tienen
características similares, en particular tienen la misma potencia media. La segunda condición es menos intuitiva, notar que es más general que la hipótesis de incoherencia ($R_{xy}\equiv 0$) considerada en el Capítulo \ref{chapter:procesos estocasticos}. Una versión  equivalente sería  $R_{yx}(\tau)=-R_{xy}(\tau)$, ya que
...
El siguiente Lema generaliza el análisis de procesos modulados del Capítulo \ref{chapter:procesos estocasticos}.
...
\begin{figure}[htb]
    ...
    \begin{blockdiagram}
        ...
        \draw [-stealth] (filtro) -- (mult);
        \draw [-stealth] (mult) -- (salida);
        \draw [-stealth] (mod) -- (mult);
    \end{blockdiagram}
...
\begin{figure}[htb]
    ...
    \begin{blockdiagram}
        ...
        \draw [-stealth] (inicio) -- (p) -- (sum);
        \draw [-stealth] (p) |- (hil);
        \draw [-stealth] (hil) -- (j);
        \draw [-stealth] (j) -| (sum);
        \draw [-stealth] (sum) -- (mult);
        \draw [-stealth] (mult) -- (salida);
        \draw [-stealth] (mod) -- (mult);
    \end{blockdiagram}
    \endpgfgraphicnamed\end{minipage}
