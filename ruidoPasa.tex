...
Para estudiar el efecto de dicho ruido en la demodulación, es conveniente tener una
representación de \emph{envolvente compleja} para el ruido:
...
Esta pregunta es en cierto sentido la inversa de la considerada en la sección anterior.
Allí se partía de un proceso estacionario de banda base, y se buscaba un proceso modulado que también fuese
estacionario. La respuesta tiene cierto grado de dificultad matemática, por lo cual la diferimos para el Apéndice A.
...
(con correlación cruzada nula).



\paragraph{Representación gráfica:} Para interpretar la expresión para la densidad espectral de
...
\begin{figure}[htb]
    ...
    \bpgf{Imagenes/ruidoPasa/ruidoPasa-4}
        \begin{tikzpicture}
        \draw [-stealth] (0,-6.5) -- (0,1.5) node [anchor=west] {$S_n(f)$};
...
Esto se deduce de lo visto en la sección anterior para el
caso de $n_i$, $n_q$ incoherentes. Es también cierto en el caso general
(remitirse al Apéndice A).
