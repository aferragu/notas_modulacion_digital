\section{Análsis de FSK y su detector coherente}
...
Suponemos $f_c T_s$ entero, sin pérdida de generalidad.
En un período de símbolo, $f_k=f_\Delta$ o $f_k=-f_\Delta$, por lo que el cambio de fase es $\Delta\phi^+=2\pi f_\Delta T_s$ o $\Delta\phi^-=-2\pi f_\Delta T_s$.
...
\subsection*{Detección FSK coherente}
 ...
 \begin{figure}[htb]
    ...
    \begin{blockdiagram}
        ...
        \draw [-stealth] (p) |- (mult1);
        \draw [-stealth] (osc1) -- (mult1);
        \draw [-stealth] (mult1) -- (int1);
        \draw [-stealth] (int1) -| node [anchor=west] {$z^+$} (sum);
        \draw [-stealth] (p) |- (mult2);
        \draw [-stealth] (osc2) -- (mult2);
        \draw [-stealth] (mult2) -- (int2);
        \draw [-stealth] (int2) -| node [anchor=west] {$z^-$} (sum);
        \draw [-stealth] (sum) -- (comp);
    \end{blockdiagram}
    \endpgfgraphicnamed}
 \end{figure}
 \FloatBarrier
...
%    \[\col{z^+(T_s)=}\]
   ...
   \begin{remarkSN}
    ...
    Como $\sen^2(\omega_c t)$ varía muy rápido, lo aproximo por su valor medio
    $\frac{1}{2}$.
    \begin{align*}
...
términos de una gráfica de $P_e$, en escala logarítmica, en
función de $\frac{\overline{E_p}}{\eta}$ en dB.
...
\begin{figure}[htb]
    ...
    \begin{tikzpicture}
        \draw [-stealth] (-0.5,0) -- (3,0) node [anchor=north] {$\frac{\overline{E_p}}{\eta}\,(dB)$};
        \draw [-stealth] (0,-0.5) -- (0,3) node [anchor=east] {$\log P_e$};
 ...
 %       \path (2.5,1) node [anchor=west] {\parbox{3cm}{curva depende (no mucho) de modul.}};
        ...
        \begin{scope}[shift={(7,0)}]
            \draw [-stealth] (-0.5,0) -- (3.5,0) node [anchor=north] {$\frac{\overline{E_p}}{\eta}\,(dB)$};
            \draw [-stealth] (0,-0.5) -- (0,3) node [anchor=east] {$\log P_e(simb)$};
            ...
            \draw [-stealth] (1.2,1) [out=45,in=180] to (3,2) node [anchor=west] {$M$};
        \end{scope}
    \end{tikzpicture}
    \endpgfgraphicnamed\end{minipage}
\end{figure}
