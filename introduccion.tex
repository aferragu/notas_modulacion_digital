...
\begin{teorema}[Shannon, 1948 -- codificación de fuente]\label{teo:shannon fuente}
...
Si, en cambio, $p\to 0$, la fuente tiene secuencias largas de $0$, y en ese caso puede ser más eficiente codificar el largo de la cadena de $0$s que indicar cada bit por separado. En el caso extremo, solo alcanza 1 bit para toda la cadena (``son todos ceros''). Lo mismo ocurre si $p\to 1$.
...
\begin{ejemplo}[Codificación por bloques]\label{ej:bloques huffman}
Consideremos una fuente independiente con tres símbolos $a_k\in\{A,B,C\}$ y distribución:
\begin{equation*}
    \p{a_k=A}=0.7, \qquad \p{a_k=B}=0.2, \qquad \p{a_k=C}=0.1.
\end{equation*}
Su entropía es:
\begin{equation*}
...
\textbf{Codificación símbolo a símbolo ($N=1$).} El código de Huffman asigna las siguientes palabras, de manera análoga al Ejemplo \ref{ej:lossless}:
\begin{equation*}
    \begin{cases}
        A \mapsto 0\\
        B \mapsto 10\\
    ...
    \end{cases}
\end{equation*}
con largo medio $\bar{L}_1 = (0.7)(1)+(0.2)(2)+(0.1)(2) = 1.3$ bits/símbolo. Esta ya es la mejor codificación posible para $N=1$ (es el árbol de Huffman óptimo). Sin embargo, aún queda lejos de $H(X)$: $\bar{L}_1 - H(X)\approx 0.143$ bits/símbolo (un $12\%$ de \emph{overhead}). El problema es de granularidad: con solo tres símbolos, no hay forma de asignarle a $A$ (con $\p{A}=0.7$, muy por encima de $1/2$) menos de $1$ bit completo.

\textbf{Codificación por pares ($N=2$).} Agrupemos los símbolos de a dos. Por independencia, $\p{a_k a_{k+1}=XY}=\p{a_k=X}\,\p{a_{k+1}=Y}$, lo que da $9$ pares posibles. Aplicando el algoritmo de Huffman a esas $9$ probabilidades obtenemos por ejemplo el siguiente código:
\begin{center}
    ...
    \end{tabular}
\end{center}
con largo medio por \emph{par} de símbolos:
\begin{align*}
    \bar{L}_2 &= (0.49)(1)+(0.14)(3)+(0.14)(3)+(0.07)(4)+(0.07)(4)\\
    &\quad+(0.04)(4)+(0.02)(5)+(0.02)(6)+(0.01)(6) = 2.33 \text{ bits/par},
\end{align*}
...
\begin{figure}[htbp]
    \centering
    \begin{blockdiagram}
        % Nodos internos (probabilidad acumulada)
        \node[operator, font=\scriptsize] (root) at (3.92,0)     {$1$};
        \node[operator, font=\scriptsize] (N7)   at (7.83,-1.3)  {$0.51$};
        \node[operator, font=\scriptsize] (N5)   at (5.16,-2.6)  {$0.23$};
        \node[operator, font=\scriptsize] (N6)   at (10.50,-2.6) {$0.28$};
        \node[operator, font=\scriptsize] (N3)   at (2.63,-3.9)  {$0.09$};
        \node[operator, font=\scriptsize] (N4)   at (7.70,-3.9)  {$0.14$};
        \node[operator, font=\scriptsize] (N2)   at (3.85,-5.2)  {$0.05$};
        \node[operator, font=\scriptsize] (N1)   at (4.90,-6.5)  {$0.03$};

        % Hojas (pares de símbolos)
        \node[block, align=center, minimum width=9mm, minimum height=8mm, font=\scriptsize] (AA) at (0.00,-1.3)  {$AA$\\$0.49$};
        \node[block, align=center, minimum width=9mm, minimum height=8mm, font=\scriptsize] (AB) at (9.80,-3.9)  {$AB$\\$0.14$};
        \node[block, align=center, minimum width=9mm, minimum height=8mm, font=\scriptsize] (BA) at (11.20,-3.9) {$BA$\\$0.14$};
        \node[block, align=center, minimum width=9mm, minimum height=8mm, font=\scriptsize] (BB) at (1.40,-5.2)  {$BB$\\$0.04$};
        \node[block, align=center, minimum width=9mm, minimum height=8mm, font=\scriptsize] (AC) at (7.00,-5.2)  {$AC$\\$0.07$};
        \node[block, align=center, minimum width=9mm, minimum height=8mm, font=\scriptsize] (CA) at (8.40,-5.2)  {$CA$\\$0.07$};
        \node[block, align=center, minimum width=9mm, minimum height=8mm, font=\scriptsize] (CB) at (2.80,-6.5)  {$CB$\\$0.02$};
        \node[block, align=center, minimum width=9mm, minimum height=8mm, font=\scriptsize] (CC) at (4.20,-7.8)  {$CC$\\$0.01$};
        \node[block, align=center, minimum width=9mm, minimum height=8mm, font=\scriptsize] (BC) at (5.60,-7.8)  {$BC$\\$0.02$};

        % Aristas con el bit correspondiente
        \draw[thick] (root) -- node[sloped,above,font=\scriptsize] {0} (AA);
        \draw[thick] (root) -- node[sloped,above,font=\scriptsize] {1} (N7);
        \draw[thick] (N7)   -- node[sloped,above,font=\scriptsize] {0} (N5);
        \draw[thick] (N7)   -- node[sloped,above,font=\scriptsize] {1} (N6);
        \draw[thick] (N5)   -- node[sloped,above,font=\scriptsize] {0} (N3);
        \draw[thick] (N5)   -- node[sloped,above,font=\scriptsize] {1} (N4);
        \draw[thick] (N6)   -- node[sloped,above,font=\scriptsize] {0} (AB);
        \draw[thick] (N6)   -- node[sloped,above,font=\scriptsize] {1} (BA);
        \draw[thick] (N3)   -- node[sloped,above,font=\scriptsize] {0} (BB);
        \draw[thick] (N3)   -- node[sloped,above,font=\scriptsize] {1} (N2);
        \draw[thick] (N4)   -- node[sloped,above,font=\scriptsize] {0} (AC);
        \draw[thick] (N4)   -- node[sloped,above,font=\scriptsize] {1} (CA);
        \draw[thick] (N2)   -- node[sloped,above,font=\scriptsize] {0} (CB);
        \draw[thick] (N2)   -- node[sloped,above,font=\scriptsize] {1} (N1);
        \draw[thick] (N1)   -- node[sloped,above,font=\scriptsize] {0} (CC);
        \draw[thick] (N1)   -- node[sloped,above,font=\scriptsize] {1} (BC);
    \end{blockdiagram}
    \caption{Árbol de Huffman para los 9 pares de símbolos del Ejemplo \ref{ej:bloques huffman} ($N=2$). Los círculos son nodos internos con su probabilidad acumulada; los rectángulos, los pares terminales con su probabilidad.}
    \label{fig:huffman bloques}
\end{figure}
...
Todos los ejemplos anteriores dejan abierta una pregunta: ¿qué tan cerca de $1$ puede estar la tasa de codificación, para una probabilidad de error dada, en un canal real? Dicho de otro modo: si la entropía nos dio el límite \emph{inferior} de bits por símbolo necesarios para representar una fuente, ¿existe un límite \emph{superior} análogo para la cantidad de información que un canal ruidoso puede transportar de forma confiable? La respuesta, debida también a Shannon en el mismo trabajo de 1948, es la segunda gran contribución de la teoría de la información, y cierra el círculo de esta introducción.

\begin{teorema}[Shannon, 1948 -- capacidad del canal AWGN]\label{teo:capacidad awgn}
Consideremos un canal continuo de ancho de banda $B$, en el que la señal se transmite con potencia media $S$ y es afectada por ruido blanco gaussiano aditivo, de potencia $N$ dentro de dicha banda. Entonces el canal tiene una \emph{capacidad}:
\begin{equation*}
...
El Teorema \ref{teo:capacidad awgn} es la contraparte exacta del Teorema \ref{teo:shannon fuente}: mientras la entropía fija cuánto se puede \emph{comprimir} una fuente sin perder información, la capacidad fija cuánto se puede \emph{transmitir} de manera confiable por un canal ruidoso. Los códigos Turbo y LDPC mencionados arriba son, precisamente, esquemas prácticos capaces de acercarse a $C$ tanto como se desee (a costa de mayor complejidad y mayor largo de bloque); la frase ``tasas cercanas a $1$ con muy baja probabilidad de error'' tiene sentido, en definitiva, en relación a esta cota.

\begin{ejemplo}[Capacidad de un canal telefónico]\label{ej:capacidad telefonica}
Una línea telefónica analógica tiene un ancho de banda de aproximadamente $B=3400\ \unit{\hertz}$, y una relación señal a ruido típica del orden de $30\ \unit{\decibel}$, es decir $S/N \approx 10^3$. Su capacidad resulta entonces:
\begin{equation*}
    C = 3400 \log_2(1+1000) \approx 3400 \times 9{,}97 \approx 33{,}9\ \unit{\kilo bps}.
\end{equation*}
Este valor no es casual: es, esencialmente, el límite que enfrentaron los módems telefónicos de los años 90 -- el estándar V.34 alcanzaba $33.6\ \unit{\kilo bps}$, muy cerca de esta cota -- y que solo pudo superarse parcialmente con los módems de $56\ \unit{\kilo bps}$, que evitaban una de las dos conversiones analógico-digitales del enlace explotando que el otro extremo era en realidad digital.
\end{ejemplo}
