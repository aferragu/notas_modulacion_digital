...
El siguiente ejemplo retoma la idea de codificar por bloques, mencionada más arriba: a diferencia del Ejemplo \ref{ej:lossless}, donde Huffman alcanzaba la entropía ya con $N=1$, ahora la fuente no es diádica y el código símbolo a símbolo se queda corto. Veremos que agrupar de a dos símbolos, tal como permite el Teorema de Shannon, acerca el largo medio a la entropía.
...
\textbf{Codificación símbolo a símbolo ($N=1$).} Huffman combina primero $B$ y $C$ ($0.2+0.1=0.3$) y luego el resultado con $A$ ($0.7+0.3=1$), dando el código:
\begin{equation*}
    \begin{cases}
        A \mapsto 0\\
        B \mapsto 10\\
    ...
    \end{cases}
\end{equation*}
con largo medio $\bar{L}_1 = (0.7)(1)+(0.2)(2)+(0.1)(2) = 1.3$ bits/símbolo. Esta ya es la mejor codificación posible para $N=1$ (es el árbol de Huffman óptimo), y sin embargo queda lejos de $H(X)$: la brecha $\bar{L}_1 - H(X)\approx 0.143$ bits/símbolo (un $12\%$ de sobrecosto) no se puede cerrar eligiendo mejor el código de un único símbolo. El problema es de granularidad: con solo tres símbolos, no hay forma de asignarle a $A$ (con $\p{A}=0.7$, muy por encima de $1/2$) menos de $1$ bit completo.

\textbf{Codificación por pares ($N=2$).} Agrupemos los símbolos de a dos. Por independencia, $\p{a_k a_{k+1}=XY}=\p{a_k=X}\,\p{a_{k+1}=Y}$, lo que da $9$ pares posibles. Aplicando Huffman a esas $9$ probabilidades:
\begin{center}
...
\end{align*}
es decir $\bar{L}_2/2 = 1.165$ bits/símbolo. La brecha con la entropía se reduce a $\bar{L}_2/2 - H(X)\approx 0.008$ bits/símbolo (un $0.7\%$ de sobrecosto): casi veinte veces más chica que con $N=1$, solo por agrupar de a dos y explotar que, aun repetido, $A$ sigue siendo mucho más probable que las combinaciones que involucran a $B$ o $C$.

Esto ilustra la cota del Teorema de Shannon: para $N=1$, $H(X)\leqslant \bar{L}_1 \leqslant H(X)+1$; para $N=2$, $H(X)\leqslant \bar{L}_2/2 \leqslant H(X)+1/2$. En ambos casos Huffman queda muy por debajo del extremo superior de la cota -- que es solo una garantía de peor caso -- y en la práctica se acerca a $H(X)$ mucho más rápido de lo que esa cota, por sí sola, deja anticipar. Aumentando $N$ seguimos acercándonos, al costo de un alfabeto de códigos cada vez más grande.
\end{ejemplo}
