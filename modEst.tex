...
densidad espectral de potencia $S_x(f)$. Para esta secci\'on suponemos tambi\'en que
...
Usando la independencia, se ve que $z(t)$ es un proceso estacionario cuya autocorrelaci\'on es el
...
Complementamos aqu\'i $x(t)$ con otro proceso $y(t)$ ``en
cuadratura",  y consideramos el proceso con envolvente compleja
$x(t) + j y(t)$, es decir
\[
u(t)=x(t)\cos(\omega_ct)  -y(t)\sen(\omega_ct).\]

Suponemos que $y(t)$ es un proceso aleatorio estacionario de iguales caracter\'isticas que
...
\begin{align*}
            ...
            \end{align*}
en el que se utiliz\'o para el \'ultimo paso una identidad trigonom\'etrica. Concluimos entonces que
el proceso $u(t)$ es estacionario, con densidad espectral de potencia
\[
S_u(f)=\frac{S_x(f-f_c)+S_x(f+f_c)}{2}.
\]
El hecho que $S_u(f)=2 S_z(f)$ se explica intuitivamente porque a trav\'es del proceso $y(t)$ se introdujo
``el doble de ruido".

De hecho, el procedimiento anterior puede generalizarse al caso en que $y(t)$ tenga correlaci\'on cruzada no nula
$R_{xy}(\tau)$, bajo cierta restricci\'on, lo cual se aplica al estudio del ruido pasabanda que se ver\'a a continuaci\'on.
Como esa generalizaci\'on es algo t\'ecnica la incluimos en el Ap\'endice A al final de estas notas.
\end{enumerate}
